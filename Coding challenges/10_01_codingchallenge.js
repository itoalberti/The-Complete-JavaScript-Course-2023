...
const poll = {
  ...
  // 1
  registerNewAnswer() {
    // 1.2 Get answer from user
    const answer = Number(prompt(`${this.question}\n${this.options.join('\n')}\n(Write option number)`));
    console.log(`answer = ${answer}`);

    // 1.3 Update 'answers' property in array 'answers'
    if (typeof answer === 'number' && answer <= this.answers.length) this.answers[answer]++;
    // 4
    this.displayResults();
...
// call = substitutes the original 'answers' array with data1 and data2
